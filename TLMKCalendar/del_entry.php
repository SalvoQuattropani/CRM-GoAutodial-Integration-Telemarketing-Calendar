...
$mail->Username = "salvo41993@gmail.com";

//Password to use for SMTP authentication
$mail->Password = "sq270693";

//Set who the message is to be sent from
$mail->setFrom('salvo41993@gmail.com', 'ADMIN');



//Set who the message is to be sent to
$mail->addAddress('salvo41993@gmail.com', 'ADMIN');


//RICAVO L'INDIRIZZO MAIL DI CHI CREA/MODIFICA 'EVENTO

 


 
$connessione=mysql_connect("localhost","3306","root","vicidialnow");
if($connessione)
{
...
$email_possessore_agenda =  mysql_query( "SELECT cal_email FROM webcal_user WHERE cal_login = '$user'");
$res_email_possessore_agenda=mysql_fetch_array($email_possessore_agenda);



$substr="@" ;
$str = $res_email_possessore_agenda[cal_email] ;
$trovato = strpos($str, $substr);

if(!$trovato){
	               //controlla se esiste @ all'interno della mail cercata perche se l'evento è notificato de un utento non admin, $user=NULL
	
$res_email_possessore_agenda[cal_email]=$login_email; 
}




if(!$email_possessore_agenda )
{
	print("<h8>-quesru fallita-</h8>");
	
}

$mail->addAddress($res_email_possessore_agenda[cal_email], $user);
...
//$do_send = get_pref_setting ( $old_participant, 'EMAIL_EVENT_DELETED' );  //CONTROLLO SE è STATO ELIMINATO L'EVENTO: SE è STATO NON  ELIMINATO ALLORA RESTITUIRA' 'Y' VICEVERSA NUlla


if($login_fullname==$user){	
$nome_utente_che_crea_evento=$user;}
else{
	$nome_utente_che_crea_evento=$login_fullname;
}
...
$minuti= 'XXX';	 
//unisco l'indirizzo per gmaps	
/*
$ubicazione = str_replace(' ', '', $location);
$locazione=('INDIRIZZO: XXX');
*/
//------------------------------------------------------RECUPERO TUTTE LE INFORMAZIONI
$descrizione_1 =  mysql_query( "SELECT cal_description FROM webcal_entry WHERE cal_id = '$id'");
$descrizione_2=mysql_fetch_array($descrizione_1);
$descrizione_3= $descrizione_2[cal_description] ;

$nome_1 =  mysql_query( "SELECT cal_name FROM webcal_entry WHERE cal_id = '$id'");
$nome_2=mysql_fetch_array($nome_1);
$nome_3= $descrizione_2[cal_name] ;

$fmtdate_1 =  mysql_query( "SELECT cal_due_date FROM webcal_entry WHERE cal_id = '$id'");
$fmtdate_2=mysql_fetch_array($fmtdate_1);
$fmtdate_3= $fmtdate_2[cal_due_date] ;
...
//CONTROLLO SE L'EVENTO E' STATO CANCELLATO. IN TAL CASO LA VARIABILE $do_send sarà vuota viceversa='Y'
...
$fmtdate = ( $timetype == 'T'
          ...
          $msg =  " <h1>APPUNTAMENTO ELIMINATO</h1><BR><BR><hr></hr><BR><BR>Salve,\n <br> "
           .  $nome_utente_che_crea_evento . "\n ha cancellato il seguente evento:<br><br>"
		   . str_replace ( 'XXX', $descrizione_3, $descStr ) . "\n <br> "
           . str_replace ( 'XXX', $name, $subjStr ) . "\n <br>"
		      . str_replace ( 'XXX', date_to_str ( $fmtdate_3 ), $dateStr ) . "\n <br><br><br>Saluti.<br><br><br>"    ;/*
			  // Apply user's GMT offset and display their TZID
     
           . str_replace ( 'XXX', $description, $descStr ) . "\n <br> "    
		  
         
		   . str_replace ( 'XXX', $location, $locazione ) . "\n <br> "
          
. $mapm  ."\n <br><br><br>MAPPA:<br><br>" 
. str_replace ( 'XXX', $ubicazione , $Mappa ). "\n <br>";
*/
...
$mail->addReplyTo($res_email_possessore_agenda[cal_email], $user);
