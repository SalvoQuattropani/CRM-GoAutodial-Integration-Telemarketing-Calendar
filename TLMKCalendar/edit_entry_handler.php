...
$remove = getPostValue ( 'remove' ); //'tre' if the event has been removed, 'else' false
...
$Mappa='https://www.google.it/maps/place/XXX';
...
//require_valide_referring_url ();// COMMENTATO PER ACCEDERE DA REMOTO
...
if ( empty ( $error ) ) {
  ...
  if ( empty ( $error ) ) {
   ...
   $from = $login_email;//mittente->colui che notifica l'evento
...
//REVUPERO LO STATO DELL'APPUNTAMENTO
...
$risultato =  mysql_query( "SELECT cat_name FROM webcal_categories WHERE cat_id = '$stato_abs'");
$res_risultato=mysql_fetch_array($risultato);

$stato_app = $res_risultato[cat_name] ;

	
}   else{
	
	$stato_app='NON CLASSIFICATO';
}   
...
$fmtdate = ( $timetype == 'T'
            ? date ( 'Ymd', $eventstart ): gmdate ( 'Ymd', $eventstart ) );
          $msg =  " <h1>APPUNTAMENTO</h1><BR><BR><hr></hr><BR><BR>Salve,\n <br>
		  "
           . str_replace ( 'XXX', $nome_utente_che_crea_evento,
            ( $newevent || ( empty ( $old_status[$participants[$i]] ) )
              ? $newAppStr : $updAppStr  ) ) . "\n <br><br>"
           . str_replace ( 'XXX', $name, $subjStr ) . "\n <br>"
           . str_replace ( 'XXX', $description, $descStr ) . "\n <br> "    
		   . str_replace ( 'XXX', $stato_app, $statoStr ) . "\n <br> "   
           . str_replace ( 'XXX', date_to_str ( $fmtdate ), $dateStr )
           . "\n <br>"
		   . str_replace ( 'XXX', $location, $locazione ) . "\n <br> "
          // Apply user's GMT offset and display their TZID
          . ( $timetype != 'T' ? ''
            : str_replace ( 'XXX',$entry_hour ,
              $timeStr )  ). ":"
. str_replace ( 'XXX', $entry_minute ,  $minuti )."\n"
...
header("location: http://192.168.1.30/TLMKCalendar/month.php"); 
